...
set default as=65500 router-id=10.0.4.2

/routing ospf instance
set [ find default=yes ] router-id=10.0.4.2

/ip address
add address=10.0.4.2/32 interface=loopback network=10.0.4.2
add address=10.0.4.102/30 interface=ether3 network=10.0.4.100
add address=10.0.7.101/30 interface=ether4 network=10.0.7.100
add address=10.0.8.102/30 interface=ether5 network=10.0.8.100
...
/routing bgp peer
add address-families=vpnv4 name=peer1 remote-address=10.0.1.2 \
    remote-as=65500 update-source=loopback
add address-families=vpnv4 name=peer2 remote-address=10.0.5.2 \
    remote-as=65500 route-reflect=yes update-source=loopback
add address-families=vpnv4 name=peer3 remote-address=10.0.6.2 \
    remote-as=65500 route-reflect=yes update-source=loopback
